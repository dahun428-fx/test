...
import styles from './AddToMyComponentsRecommend.module.scss';
...
import {
	...
	RecommendItem,
} from '@/models/api/cameleer/getMyPartsListAddModalRecommend/GetMyPartsListAddModalRecommendResponse';
import { assignListParam } from '@/utils/cameleer';
import { getHeight } from '@/utils/dom';
import { toNumeric } from '@/utils/string';

/** modal 内のリストのため画面リサイズで増減しない */
const PAGE_SIZE = 5;

type Props = {
	recommend: GetMyPartsListAddModalRecommendResponse;
	className?: string;
};
...
export const AddToMyComponentsRecommend: React.VFC<Props> = ({
	...
	return (
		<div className={classNames(styles.recommendContainer, className)}>
			...
			<p className={styles.supplementaryMessage}>
				{t(
					'pages.productDetail.addToMyComponentsModal.addToMyComponentsRecommend.supplementaryMessage'
				)}
			</p>
							...
							>
								<div className={styles.item}>
									...
									<p className={styles.price}>
										{t(
											'pages.productDetail.addToMyComponentsModal.addToMyComponentsRecommend.unitPrice'
										)}
										<StandardPrice
									...
									<p className={styles.daysToShip}>
										{t(
											'pages.productDetail.addToMyComponentsModal.addToMyComponentsRecommend.daysToShip'
										)}
										<CrmDaysToShip
...
AddToMyComponentsRecommend.displayName = 'AddToMyComponentsRecommend';
