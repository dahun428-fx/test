...
import { DownloadProductDetailResult } from '@/components/pc/pages/ProductDetail/ProductDetailsDownloadModal/ProductDetailsDownloadContent';
...
export const useProductDetailsDownloadContent = (
	...
	const quantityAlert = useCallback(
		({ minQuantity = 1, orderUnit = 1 }: Price): string => {
			if (minQuantity > 1 && orderUnit > 1) {
				return t(
					'pages.productDetail.productDetailsDownloadModal.quantityAlertModal.bothFactorValidation',
			...
			if (minQuantity > 1) {
				return t(
					'pages.productDetail.productDetailsDownloadModal.quantityAlertModal.minQuantityOnlyValidation',
					{ minQuantity }
				);
			}
			...
			if (orderUnit > 1) {
				return t(
					'pages.productDetail.productDetailsDownloadModal.quantityAlertModal.orderUnitOnlyValidation',
					{ orderUnit }
				);
			}
	...
	const handleClearPriceCheckError = useCallback(() => {
		if (isOtherCadSiteType && priceRef.current) {
			downloadZip({
				priceResponse: priceRef.current,
			});
			close?.();
	...
	}, [close, downloadZip, isOtherCadSiteType]);
