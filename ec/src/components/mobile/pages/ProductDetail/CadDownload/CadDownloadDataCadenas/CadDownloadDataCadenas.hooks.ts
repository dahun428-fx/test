...
import { ErrorType } from '@/components/mobile/pages/ProductDetail/CadDownload/types';
...
import {
	addItemWithoutLocalStorageOperation,
	selectCadDownloadItems,
...
export const useCadDownloadDataCadenas = ({
	...
	const cadDownloadStack = useSelector(selectCadDownloadItems);

	const [loadingResolve, startToLoadResolve, endLoadingResolve] =
		useBoolState(true);
	const [generating, startToGenerate, endGenerating] = useBoolState(false);
	const [showsProgress, setShowsProgress] = useBoolState(false);
	...
	const startDownload = useCallback(
		(xmlFile: string) => {
			...
			addItemWithoutLocalStorageOperation(dispatch)({
	...
	const startGeneratingMonitoring = useCallback(async () => {
		try {
			while (generating) {
				await sleep(1000);
				if (cadDownloadStack[0]?.status === CadDownloadStatus.Done) {
					endGenerating();
				}
			}
		} catch {
	...
	}, [cadDownloadStack, endGenerating, generating]);
	...
	return {
		...
		loadingResolve,
		showsProgress,
		downloadItem: cadDownloadStack[0],
		generateRef,
		resolveRef,
		resolveIFrameName,
		generateIFrameName,
