...
/** 型番検索APIレスポンス */
export interface SearchPartNumberResponse$search extends MsmApiResponse {
	/**
	 * 総件数
	 * - 検索にヒットした総件数
	 * - example: 1022
...
export interface PartNumber {
	 ...
	 * - field groups: @default, @search, @detail
	 * NOTE: 外部設計書上は必須ではない。ect-api チームに確認済み。
	 */
	partNumber: string;
	/**
	 * UFフラグ
...
export interface AlterationSpec {
	 ...
	 *         は返却しない
	 */
	specViewType: AlterationSpecViewType;
	/**
	 * 補足表示タイプ
	 * - スペックの補足表示種別
	 *   1: 通常
	 ...
	 * - field groups: @search
	 */
	numericSpec: AlterationNumericSpec;
	/**
	 * スペック値リスト
	 * - field groups: @search
	 */
...
/** 数値入力項目 */
export interface AlterationNumericSpec {
	/**
	 * 非表示フラグ
	 * - 数値入力項目を表示するかどうか
	 *   0: 表示する
 ...
 *   9: リスト選択形式
 */
export type AlterationSpecViewType = Extract<
	SpecViewType,
	'2' | '3' | '4' | '8' | '9'
>;
